...
namespace Unity.FPS.Game
{
    ...
    public class WeaponController : MonoBehaviour
    {
        ...
        [Header("Melee / Hitscan")]
        [Tooltip("Si el arma es cuerpo a cuerpo / hitscan (no usa proyectil)")]
        public bool IsMelee = false;


        [Tooltip("Distancia máxima del golpe (metros) para melee/hitscan)")]
        public float MeleeRange = 2f;

        [Tooltip("Radio para el golpe tipo 'sweep' (si > 0 usa SphereCast/OverlapSphere)")]
        public float MeleeRadius = 0.5f;

        [Tooltip("Capas que pueden ser atacadas")]
        public LayerMask HitLayerMask = ~0; // por defecto todo

        [Tooltip("Si true usa la camera center como origen/dirección, si false usa WeaponMuzzle")]
        public bool UseCameraForwardForHit = true;

        [Tooltip("Daño que aplica el golpe melee")]
        ...
        // {
        //     RaycastHit hit;
        //     if (Physics.Raycast(transform.position, transform.forward, out hit, MeleeRange, HitLayerMask))
        //     {
        ...
        //     Debug.Log("Ataque melee ejecutado");
        // }


        // PUBLIC: permite que otros scripts (ej: EnemyController) invoquen un melee desde cualquier punto/dirección
        public void PerformMeleeAttackFrom(Vector3 origin, Vector3 direction)
{
    float meleeRange = MeleeRange;
    float meleeRadius = MeleeRadius;
    LayerMask layerMask = HitLayerMask;

    // Usa cámara si quieres que el ataque sea más fácil
    if (UseCameraForwardForHit && Camera.main != null)
    ...
    {
        // SphereCast primero
        if (Physics.SphereCast(origin, meleeRadius, direction, out RaycastHit hit, meleeRange, layerMask, QueryTriggerInteraction.Ignore))
        {
            hitColliders = new Collider[] { hit.collider };
        }
        else
        {
            // OverlapSphere como fallback
            hitColliders = Physics.OverlapSphere(origin + direction * (meleeRange * 0.5f), meleeRadius, layerMask, QueryTriggerInteraction.Ignore);
        }
    }
    else
    {
        if (Physics.Raycast(origin, direction, out RaycastHit hit, meleeRange, layerMask, QueryTriggerInteraction.Ignore))
            hitColliders = new Collider[] { hit.collider };
    }
    ...
    if (hitColliders != null && hitColliders.Length > 0)
    {
        ...
        {
            if (Owner != null && targetCollider.transform.root == Owner.transform)
                continue; // evitar auto-daño

            var health = targetCollider.GetComponentInParent<Health>();
            if (health != null)
                health.TakeDamage(MeleeDamage, Owner);
            else
            {
                var id = targetCollider.GetComponentInParent<IDamageable>();
                if (id != null)
                    id.ApplyDamage(MeleeDamage, Owner);
            }

            Debug.DrawLine(origin, targetCollider.ClosestPoint(origin), Color.red, 0.5f);
            Debug.Log($"Melee hit: {targetCollider.name}");
        }
    }
    else
    {
    ...
    // Anim/sonido/eventos del WeaponController
    if (WeaponAnimator)
