...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerCharacterController : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Ataque melee con click izquierdo
            if (Keyboard.current.rKey.wasPressedThisFrame)
            {
                WeaponController currentWeapon = m_WeaponsManager.GetActiveWeapon();
                if (currentWeapon != null)
                {
                    // Origen del melee: la boca del arma si existe, sino el frente del jugador
                    Vector3 origin = currentWeapon.WeaponMuzzle != null
                        ? currentWeapon.WeaponMuzzle.position
                        : transform.position + transform.forward * 0.5f;

                    // Dirección hacia adelante del jugador
                    Vector3 targetPos = transform.position + transform.forward * 2f; // 2 metros adelante
                    Vector3 dir = (targetPos - origin).normalized;

                    // Llamamos al método que recibe origen y dirección
                    currentWeapon.PerformMeleeAttackFrom(origin, dir);

                    Debug.Log("Melee ejecutado desde el jugador");
                }
            }
